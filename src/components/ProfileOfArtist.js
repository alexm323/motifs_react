...
useEffect(() => {
    ( async () => {
        axios.get(`https://motifs-server.herokuapp.com/getProfileOfArtist/${artistId.id}`, {headers: {Authentication: loggedInUserContext?.accesstoken}})
        .then(({ data }) => {
            setFeed({
                dataBack: data,
                posts: data.posts,
        ...
        <div>
            ...
            <div className='gradient landing'>
                ...
                <div className='profileHead'>
                    ...
                    {/* <div className='profileNav radius'>
                        <div className='motifsHead padding flex'>
    
                                <h1>Motifs</h1>
                        </div>
                        
                            <div className='padding border-bottom subNav'>
                                <Link to='/feed'>
                                    <h2>Library</h2>
                                </Link>
                            </div>
                            ...
                            <div className='padding subNav subLogout' onClick={logout}>
                                <h2>Log out</h2>
                            </div> */}
                        ...
                        {/* <div className='padding'>
                            <ButtonHash link={`#music`} name={`Audio`} classes={`trackBtn paddingSmall`}/>
                            <Button link={`/feed`} name={`Library`} classes={`trackBtn paddingSmall margin-top`}/>
                            <Button link={`/upload`} name={`Upload`} classes={`trackBtn paddingSmall margin-top`}/>
                            <Button link={`/`} name={`Logout`} classes={`trackBtn paddingSmall margin-top`} onClick={logout}/>    
                        </div> */}
                    {/* </div> */}
                </div>
            </div>
